...
export const getLikedPosts = async (req, res) => {
	...
	try {
		...
		const likedPosts = await Post.find({ _id: { $in: user.likedPosts } })//({_id:{$in:[....]}})
			.populate({
				path: "user",
				select: "-password",
			})
...
export const getFollowingPosts = async (req, res) => {
	try {
		...
		const feedPosts = await Post.find({ user: { $in: following } })//({user:{$in:following}})
		//amongst all posts we l get the posts with user matching with following array
			.sort({ createdAt: -1 })
			.populate({
				path: "user",
				select: "-password",
